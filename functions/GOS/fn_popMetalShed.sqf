...
_dirList = [180, 270, 315, 315, 315];
{
	if(random 1 > 0.2) then {
    	_space = (_shed modelToWorld (_posList select _forEachIndex)) isFlatEmpty [3, -1, -1, 1, -1, false, _shed];
		if(count _space > 0) then {
			_v = createVehicle [_x, ASLToAGL _space, [], 0, "NONE"];
...
_group = [getPos _shed, east, ["O_Soldier_F", "O_Soldier_F"]] call BIS_fnc_spawnGroup;
